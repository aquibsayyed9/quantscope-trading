...
const (
	MAX_ORDERS_PER_SIDE = 100000 // orders per side (buy/sell)
	MAX_TOTAL_ORDERS    = 200000 // Total orders per symbol
...
func (obm *OrderBookManager) GetOrderBook(symbol string) (*OrderBook, error) {
	...
	buyHeap := &OrderHeap{orders: make([]*shared.Order, 0, MAX_ORDERS_PER_SIDE), isBuy: true}
	sellHeap := &OrderHeap{orders: make([]*shared.Order, 0, MAX_ORDERS_PER_SIDE), isBuy: false}
	heap.Init(buyHeap)
	heap.Init(sellHeap)
...
func (obm *OrderBookManager) restoreOrderBookFromRedis(symbol string) error {
	ctx := context.Background()
	buyKey := fmt.Sprintf("orderbook:%s:buy", symbol)
	sellKey := fmt.Sprintf("orderbook:%s:sell", symbol)
...
func (obm *OrderBookManager) getTradeFromRedis(tradeID string) (*shared.Trade, error) {
	...
	trade := &shared.Trade{
		...
		MakerSide:   shared.Side(tradeData["maker_side"]),
